...
- Notes may begin with Bear YAML frontmatter: a strict top-of-note `--- ... ---` block that sits above the markdown title. Treat that frontmatter as a first-class pre-title segment that is independent from template awareness. Reads should expose it separately from editable content, and ordinary body insertions should still target editable content rather than the frontmatter block.
- `bear_replace_content` computes the final full note markdown locally, then commits through Bear's full replacement path. `expected_version` is required only for full-body replacement and must come from a trusted full-note lineage: `bear_get_notes`, `bear_create_notes`, or a later verified note-mutation receipt that descended from one of those full-note states. Ursus tracks that replace-eligibility separately from its short-lived cached editable-content snapshots. If the note is still on the same trusted version, body replacement may proceed even after the snapshot expires. If Bear advances the note's DB version without changing editable content and Ursus still has a matching trusted snapshot, Ursus can rebase that trusted lineage onto the current version and continue without a reread. If the note changed and Ursus still has the earlier editable content cached, it can return a compact conflict receipt with bounded diff hunks plus a short-lived single-use `conflictToken` so the same body replacement can be retried without a separate reread. If the note changed after the trusted version and the earlier cached content has expired, Ursus requires a fresh `bear_get_notes` before retrying. Title/frontmatter/string replacements ignore `expected_version` and `conflict_token` when clients send them anyway.
- Discovery tools should return compact note summaries; `bear_get_notes` remains the full-note fetch and now includes Bear's current note revision `version`, `frontmatter.content` when present, and explicit-title detection for titleless notes. Discovery `matchedFields` can report `frontmatter` separately from `body` when a text hit comes from the top-of-note YAML block.
- Confirmed `bear_create_notes` receipts may include Bear's current note revision `version` when the created note can be verified immediately after the x-callback round trip. That verified create version starts trusted full-note lineage for later descendant mutation receipts and full-body replacement. Creation receipts may also include `opened` plus `openedIn` (`main_window` or `new_window`) so callers can tell whether the config-driven create flow already brought the note to the foreground.
- Verified note-mutation receipts (`bear_insert_text`, `bear_add_files`, `bear_add_tags`, `bear_remove_tags`, `bear_apply_template`, `bear_restore_notes`, and `bear_replace_content` title/body/frontmatter/string success receipts) may include a new Bear `version` only when the targeted note already descends from trusted full-note lineage. Cold note mutations do not start that lineage on their own. If Bear silently bumps the note version while editable content stays identical and Ursus still has a trusted snapshot for that content, descendant mutations can rebase the lineage to the current version and continue returning trusted versions.
...
- Foreground handoff requests that originate from a background-found update must preserve background-check semantics on retry so a stale handoff cannot show Sparkle's "up to date" alert.
- Sparkle-only foreground handoff now uses a stronger foreground activation request and Sparkle's gentle scheduled-update reminder hooks for background-found updates. Ursus still lets Sparkle handle near-launch scheduled alerts that already qualify for immediate focus, but it takes over deferred scheduled alerts and brings the already-found update into focus with at most one delayed plus one wake-time presentation. Keep those reminders scoped to the single update session and cancel them when the user gives attention or the update cycle finishes so scheduled checks never turn into repeated reminders.
...
- Background-found handoffs use the same silent Sparkle retry path as before, but the foreground app now opts into Sparkle gentle reminders, keeps immediate-focus scheduled alerts on Sparkle's default path, and takes over deferred scheduled alerts with a single delayed / wake-time in-focus presentation of the already-found update.
