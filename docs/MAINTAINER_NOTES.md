...
- `bear_replace_content` computes the final full note markdown locally, then commits through Bear's full replacement path. Full-body replacement normally uses `expected_version` from `bear_get_notes` as a stale-read guard. When Ursus still has the exact earlier body text for that stale version, it may return a compact conflict receipt with bounded diff hunks plus a short-lived single-use `conflictToken` so the same body replacement can be retried without a separate reread. Title/string replacements ignore `expected_version` and `conflict_token` when clients send them anyway.
