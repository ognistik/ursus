...
- `bear_replace_content` computes the final full note markdown locally, then commits through Bear's full replacement path. Full-body replacement now requires `expected_version` from a trusted full-note lineage: `bear_get_notes`, `bear_create_notes`, or a later verified note-mutation receipt that descended from one of those full-note states. Ursus tracks that replace-eligibility separately from its short-lived cached editable-content snapshots. If the note is still on the same trusted version, body replacement may proceed even after the snapshot expires. If Bear advances the note's DB version without changing editable content and Ursus still has a matching trusted snapshot, Ursus can rebase that trusted lineage onto the current version and continue without a reread. If the note changed and Ursus still has the earlier editable content cached, it can return a compact conflict receipt with bounded diff hunks plus a short-lived single-use `conflictToken` so the same body replacement can be retried without a separate reread. If the note changed after the trusted version and the earlier cached content has expired, Ursus requires a fresh `bear_get_notes` before retrying. Title/string replacements ignore `expected_version` and `conflict_token` when clients send them anyway.
