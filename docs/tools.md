...
*   **`bear_find_notes`**: The primary search tool. Filter by text (including attachments), tags, date ranges, pinned status, todos, or whether the note is in your archive. When a note has Bear YAML frontmatter at the very top, compact summaries may include `hasFrontmatter` and a short `frontmatterSnippet`. Body searches match frontmatter content plus editable body content, but not the note title, and `matchedFields` can report `frontmatter` separately from `body`. You can request AI to set the `text_mode` to `substring`, `any_terms`, or `all_terms` for flexible matching. If you have multiple topics or alternate phrasings that may belong to different notes, AI should prefer separate objects in `operations` instead of one combined text string.
...
*   **`bear_replace_content`**: Perform structural replacements.
    *   **Kinds**: Choose `title`, `body` (replaces the entire editable area), `frontmatter` (replaces or removes the Bear YAML frontmatter block), or `string` (replaces specific text).
    *   **Frontmatter editing**: For `frontmatter`, pass only the content that goes *inside* the leading/trailing `---` markers. Ursus manages those fences automatically and will normalize them away if they are accidentally included in `new_string`. Pass an empty string to remove the frontmatter block entirely.
    ...
    *   **Forgiving extra field behavior**: If `expected_version` is accidentally sent with `title`, `frontmatter`, or `string`, Ursus ignores it instead of failing the operation.
    *   **Forgiving retry field behavior**: If `conflict_token` is accidentally sent with `title`, `frontmatter`, or `string`, Ursus ignores it instead of failing the operation.
