...
*   **`bear_find_notes`**: The primary search tool. Filter by text (including attachments), tags, date ranges, pinned status, todos, or whether the note is in your archive. When a note has Bear YAML front matter at the very top, compact summaries may include `hasFrontMatter` and a short `frontMatterSnippet`. Body searches match front matter content plus editable body content, but not the note title, and `matchedFields` can report `front_matter` separately from `body`. You can request AI to set the `text_mode` to `substring`, `any_terms`, or `all_terms` for flexible matching. If you have multiple topics or alternate phrasings that may belong to different notes, AI should prefer separate objects in `operations` instead of one combined text string.
...
*   **`bear_get_notes`**: Fetch the full note content, current note `version`, and attachment OCR content. If a note has Bear YAML front matter, Ursus returns that separately as `frontMatter.content` and keeps editable `content` focused on the note body. It also reports `hasExplicitTitle` so titleless notes can be distinguished from normal titled notes. Use this when you need the current text body or the fresh Bear revision number for a guarded full-body replacement. If you are just trying to find a note to edit, you don't need to call this—the mutation tools resolve note targets automatically. Set `include_attachment_text` to `true` if you need to access OCR text from attachments.
...
*   **`bear_replace_content`**: Perform structural replacements.
    *   **Kinds**: Choose `title`, `body` (replaces the entire editable area), `front_matter` (replaces or removes the Bear YAML front matter block), or `string` (replaces specific text).
    *   **Front matter editing**: For `front_matter`, pass only the content that goes *inside* the leading/trailing `---` markers. Ursus manages those fences automatically and will normalize them away if they are accidentally included in `new_string`. Pass an empty string to remove the front matter block entirely.
    *   **Version guard for full-body replace**: `expected_version` is required only when replacing `body`, and it should come from the latest `bear_get_notes` read of that same note. This prevents a stale full-note overwrite if Bear content changed after the read.
    ...
    *   **Forgiving extra field behavior**: If `expected_version` is accidentally sent with `title`, `front_matter`, or `string`, Ursus ignores it instead of failing the operation.
    *   **Forgiving retry field behavior**: If `conflict_token` is accidentally sent with `title`, `front_matter`, or `string`, Ursus ignores it instead of failing the operation.
