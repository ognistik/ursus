...
- `bear_replace_content` computes final full note markdown locally and commits through Bear's `replace_all` mode. Title edits rebuild the note with the new title; body/string edits are limited to editable content. Full-body replacement normally requires `expected_version` from a fresh `bear_get_notes` read so stale reads do not overwrite newer Bear edits. When Ursus still has the exact earlier body text for that stale version, it can return a compact conflict receipt with bounded diff hunks plus a short-lived single-use `conflictToken` that allows retrying the same body replacement without a separate reread. If the diff is too large or too truncated to summarize safely, Ursus returns a conflict receipt that forces a fresh `bear_get_notes`. `expected_version` and `conflict_token` are otherwise tolerated and ignored for title/string replacement.
- `bear_create_notes` can return Bear's current note `version` in its compact creation receipt when the x-callback transport successfully verifies the created note against Bear's database immediately after creation. That same receipt can also report whether the note was opened and whether it opened in Bear's `main_window` or a `new_window`, based on the effective create presentation settings already applied to the x-callback request.
