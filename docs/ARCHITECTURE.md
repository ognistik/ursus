...
- `bear_add_files` now follows the same template-aware planning model: if the current note matches the active `template.md`, the service inserts a temporary backend-only header anchor inside the extracted `{{content}}` region, targets Bear's `add-file` call at that header using `prepend`, and then removes the temporary header with a final `replace_all` write so the attachment lands inside the template content boundaries. Top/bottom placement is controlled by where the anchor is inserted, not by Bear's header-targeted append behavior. Non-templated notes still fall back to Bear's direct prepend/append add-file path.
