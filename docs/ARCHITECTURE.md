...
- Embedded CLI runs launched through the app bundle receive a Sparkle update-checking hook from the app target. `ursus mcp` and `ursus bridge serve` start Sparkle's scheduled check cycle for the long-running MCP surfaces using a background `SPUUpdater` user driver that does not register the bridge as the visible app, while `ursus --check-updates` hands off to the foreground Ursus app for a user-initiated Sparkle check with the standard user driver. Ordinary short-lived CLI commands do not participate in Sparkle's scheduled checks.
