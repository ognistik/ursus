...
- Long-running bridge / stdio processes are expected to keep Sparkle scheduling alive for days at a time. Sparkle maintains its own next-check timer inside that running process, so leaving bridge / MCP running should continue daily checks without requiring the dashboard app to stay open.
