...
- `bear_add_tags` and `bear_remove_tags` are note-scoped full-replacement mutations. When a matched template contains `{{tags}}`, they update that literal tag slot; otherwise they operate on the raw body by removing literal tag tokens with whitespace cleanup and by extending an existing tag-only cluster or appending one canonical tag line at the end of the note body.
