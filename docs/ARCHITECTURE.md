...
- Discovery summaries include template-aware snippets, `hasAttachments`, matched fields for text searches, optional `hasBackups`, and front matter metadata when a note has a strict Bear YAML front matter block at the very top. Attachment OCR/search text never appears in discovery output.
...
- Discovery summaries stay compact: they expose `hasFrontMatter`, may include a short `frontMatterSnippet`, and can report `matchedFields: ["front_matter"]` separately from `["body"]` when the text hit came from the front matter block.
