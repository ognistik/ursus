...
- Discovery summaries include template-aware snippets, `hasAttachments`, matched fields for text searches, optional `hasBackups`, and frontmatter metadata when a note has a strict Bear YAML frontmatter block at the very top. Attachment OCR/search text never appears in discovery output.
- `bear_get_notes` returns a canonical editable `content` field, separate `frontmatter.content` when that top-of-note Bear YAML block is present, `hasExplicitTitle` for titleless-note handling, the current Bear note `version` from Bear's SQLite row revision (`Z_OPT`), strips template wrapper noise when the current template matches, and returns attachment metadata by default. Attachment OCR/search text is opt-in through `include_attachment_text: true`. A `bear_get_notes` result also seeds trusted full-note lineage for that note version, which later descendant mutation receipts and full-body replacements may continue.
- Discovery summaries stay compact: they expose `hasFrontmatter`, may include a short `frontmatterSnippet`, and can report `matchedFields: ["frontmatter"]` separately from `["body"]` when the text hit came from the frontmatter block.
...
- Notes are modeled structurally as optional top-of-note Bear YAML frontmatter, optional explicit markdown title, then editable body. Template awareness sits inside the body portion only; frontmatter is independent from templates and ordinary insertion tools still target editable body content.
- `bear_replace_content` computes final full note markdown locally and commits through Bear's `replace_all` mode. Title edits rebuild the note with the new title; `frontmatter` edits replace or remove only the top-of-note Bear YAML block and treat `new_string` as the inner YAML content without the surrounding `---` fences; body/string edits are limited to editable content. `expected_version` is required only for full-body replacement and must come from trusted full-note lineage for that note: a prior `bear_get_notes`, a verified `bear_create_notes` receipt for that created note, or a later verified descendant mutation receipt that continued the same lineage. Ursus tracks that replace-eligible lineage separately from its short-lived editable-content snapshot cache. If the note is still on the same trusted version, body replacement may proceed even when the earlier cached editable content has expired. If Bear silently bumps the note version while editable content remains identical and Ursus still has the matching trusted snapshot, Ursus can rebase that trusted lineage to the current version and proceed without forcing a reread. When the note changed and Ursus still has the earlier editable content cached, it can return a compact conflict receipt with bounded diff hunks plus a short-lived single-use `conflictToken` that allows retrying the same body replacement without a separate reread. If the note changed after the trusted version and the earlier cached editable content has expired, Ursus returns a conflict receipt that forces a fresh `bear_get_notes`. `expected_version` and `conflict_token` are otherwise tolerated and ignored for title/frontmatter/string replacement.
- `bear_create_notes` can return Bear's current note `version` in its compact creation receipt when the x-callback transport successfully verifies the created note against Bear's database immediately after creation. That verified create version starts trusted full-note lineage for later descendant mutation receipts and full-body replacement. The same receipt can also report whether the note was opened and whether it opened in Bear's `main_window` or a `new_window`, based on the effective create presentation settings already applied to the x-callback request.
- Verified note-mutation receipts return a new Bear `version` only when the targeted note already descends from trusted full-note lineage. Cold note mutations can still succeed, but they do not start full-body replacement lineage on their own. When Bear advances a note's DB version without changing editable content and Ursus still has a matching trusted snapshot, descendant mutations can rebase the lineage to that current version and continue returning trusted versions.
...
- Sparkle behavior is intentionally split by initiation mode. User-initiated checks (`ursus update check` and the app UI's "Check for Updates…") may show Sparkle's normal "no updates available" result. Background bridge / stdio scheduled checks stay silent when no update is found or when the check errors; they only foreground Sparkle when an update is actually available. When a background-found update is re-checked during the foreground handoff, that retry must also stay on Sparkle's silent background-check path so stale handoffs never surface a spurious "no updates available" alert.
- Sparkle-only handoff uses a stronger activation request than the ordinary dashboard path and opts into Sparkle's gentle scheduled-update reminders for background-found updates. Ursus lets Sparkle handle near-launch scheduled alerts that already qualify for immediate focus, but for deferred scheduled alerts Ursus takes over the single-session handoff, keeps the re-check on Sparkle's silent background path, and performs at most one delayed plus one wake-time in-focus presentation of the already-found update. Those reminder hooks are bounded to the one update session and are canceled once the user gives attention or the session finishes so visibility improves without becoming a recurring reminder loop.
...
- Editable-content snapshots used for stale-version diff/conflict summaries are in-memory, session-local, and short-lived; once they expire, Ursus can still honor a matching trusted `expected_version`, but stale trusted versions require a fresh `bear_get_notes` instead of diff-based conflict guidance. Those same snapshots are also what let Ursus safely rebase trusted lineage across benign DB-version drift when the editable content is unchanged.
