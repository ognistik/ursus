...
- Debug builds add hidden non-public donation-testing CLI flags that write to the same runtime-state SQLite store, while release builds keep the production threshold-only behavior.
...
- Bridge HTTP request tracing now writes compact ingress/egress lines to `Logs/debug.log`, including route, JSON-RPC method when detectable, response status/duration, and a specific `base-url-miss` hint when a client hits `/` instead of the configured MCP endpoint.
...
2. Decide whether any post-Phase-5 bridge diagnostics need a lightweight authenticated probe or whether the current transport-up plus OAuth-challenge model is sufficient.
