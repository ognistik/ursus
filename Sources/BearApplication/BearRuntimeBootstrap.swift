...
public enum BearRuntimeBootstrap {
    ...
    public static func loadConfiguration(
        ...
        BearDebugLog.append(
            "config.loaded path=\(configFileURL.path) runtimeGeneration=\(configuration.runtimeConfigurationGeneration) inboxTags=\(configuration.inboxTags) createAddsInboxTagsByDefault=\(configuration.createAddsInboxTagsByDefault) tagsMergeMode=\(configuration.tagsMergeMode.rawValue) createOpensNoteByDefault=\(configuration.createOpensNoteByDefault) openUsesNewWindowByDefault=\(configuration.openUsesNewWindowByDefault) defaultDiscoveryLimit=\(configuration.defaultDiscoveryLimit) defaultSnippetLength=\(configuration.defaultSnippetLength) backupRetentionDays=\(configuration.backupRetentionDays) disabledTools=\(configuration.disabledTools.map(\.rawValue)) bridgeEnabled=\(configuration.bridge.enabled) bridgeHost=\(configuration.bridge.host) bridgePort=\(configuration.bridge.port) bridgeAuthMode=\(configuration.bridge.authMode.rawValue)"
        )
