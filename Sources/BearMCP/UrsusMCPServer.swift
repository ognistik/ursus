    ...
    static func makeTools(configuration: BearConfiguration, selectedNoteSupported: Bool) -> [Tool] {
        [
            batchedDiscoveryTool(
                ...
                description: "Find Bear notes with text, tag, inbox-tag, and date filters and return compact summaries. Summaries may include front matter metadata when a note has a Bear YAML front matter block above its title, and `matchedFields` may report `front_matter` separately from `body`. Always call this tool with a top-level `operations` array. Within one operation, use `text_mode: any_terms` when multiple terms are alternative matches and may appear in different notes. Use `text_mode: all_terms` only when every term should appear in the same note. For multiple distinct topics, alternate phrasings, or separate search hypotheses that should stay independently identifiable in the results, prefer separate objects in `operations` instead of one combined text string. Use `bear_list_tags` first when the exact tag name is uncertain. Discovery excludes trash.\(backupDiscoverabilityHint(configuration: configuration))",
                operationProperties: findNotesOperationProperties(configuration: configuration),
                required: []
            ),
            Tool(
                name: "bear_get_notes",
                description: "Fetch full Bear note records for one or more selectors. Returned notes include the current Bear note `version` from Bear's database row revision field; use that `version` as the trusted concurrency token for a later full-body replacement or for descendant mutation receipts that continue the same full-note lineage, not as a backup or snapshot identifier. When a note has Bear YAML front matter at the very top, `bear_get_notes` returns that front matter separately from the editable `content` as `frontMatter.content`. Use this tool only when current note content, note version, attachment metadata, front matter, or other full-note fields are needed. Attachment OCR/search text is omitted unless `include_attachment_text` is `true`. Do not call it only to resolve a selector before a note-targeting mutation; those tools already resolve selectors server-side. Selectors are matched as exact note ids first, then exact case-insensitive titles.\(backupDiscoverabilityHint(configuration: configuration))\(selectedNoteDescriptionSuffix(selectedNoteSupported))",
                inputSchema: .object([
                    "type": .string("object"),
            ...
            batchedMutationTool(
                ...
                description: "Replace Bear note content while preserving note structure. For each operation, provide exactly one note target: `note` or `selected: true`. Provide `kind` and `new_string`. For `kind: string`, also provide `old_string` and `occurrence`. For `kind: body`, `expected_version` is required unless you are retrying a previous version-mismatch conflict with a server-issued `conflict_token`. For `kind: front_matter`, `new_string` is only the inner YAML content; Ursus manages the surrounding `---` fences automatically and removes accidental duplicated fences if they are included anyway. If the user asks to add, edit, or remove YAML front matter, use `kind: front_matter`, not `kind: body`. For `kind: title`, omit `old_string` and `occurrence`. `expected_version` and `conflict_token` are ignored for `kind: title`, `kind: front_matter`, and `kind: string`. Do not call `bear_get_notes` only to resolve the note selector; this tool already resolves selectors server-side. Use `bear_get_notes` first when the exact current text or current note version is not already known.",
                operationProperties: [
                    "note": noteSelectorProperty(selectedNoteSupported: selectedNoteSupported),
                    "kind": .object([
                        "type": .string("string"),
                        "enum": .array([.string("title"), .string("body"), .string("front_matter"), .string("string")]),
                        "description": .string("Required replacement kind. `title` replaces only the note title. `body` replaces the full editable body and requires `expected_version` from the latest trusted full-note lineage for that note, unless a prior conflict receipt supplied a valid `conflict_token`. Do not use `body` to add or edit front matter. `front_matter` replaces or removes only the Bear YAML front matter block that sits above the title; `new_string` should contain only the inner YAML content, not the surrounding `---` fences. `string` replaces exact text within the editable body only."),
                    ]),
                    "old_string": .object([
                        "type": .string("string"),
                    ...
                    "new_string": .object([
                        "type": .string("string"),
                        "description": .string("Required replacement text. For `kind: title`, this is the full new title and must not be empty. For `kind: body`, this is the full replacement editable body and may be empty. For `kind: front_matter`, this is only the content inside the front matter block; omit the surrounding `---` fences and pass an empty string to remove the block. For `kind: string`, this is the replacement for matched text and may be empty."),
                    ]),
                    "expected_version": .object([
                        "type": .string("integer"),
                        "description": .string("Required only for `kind: body`. Pass the exact `version` returned by the latest `bear_get_notes` read of the same note, by `bear_create_notes` for that created note, or by a later verified note-mutation receipt that descended from one of those full-note results. You may omit it only when retrying a version conflict with a valid server-issued `conflict_token`. For `kind: title`, `kind: front_matter`, and `kind: string`, this field is ignored."),
                    ]),
                    "conflict_token": .object([
                        "type": .string("string"),
                        "description": .string("Use this only with `kind: body` after Ursus returns a version-mismatch conflict receipt that includes a `conflictToken`. The token is short-lived, single-use, and valid only for retrying the same requested full-body replacement against the exact conflict state Ursus already summarized. For `kind: title`, `kind: front_matter`, and `kind: string`, this field is ignored."),
                    ]),
                ].merging(selectedNoteOperationProperty(selectedNoteSupported: selectedNoteSupported), uniquingKeysWith: { current, _ in current }),
