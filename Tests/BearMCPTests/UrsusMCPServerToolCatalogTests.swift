...
func toolCatalogInjectsCurrentSessionDefaultsIntoOverrideableFields() throws {
    ...
    #expect(try #require(replace.description).contains("`kind: frontmatter`"))
    #expect(try #require(replace.description).contains("use `kind: frontmatter`, not `kind: body`"))
    #expect(try #require(replace.description).contains("ignored for `kind: title`, `kind: frontmatter`, and `kind: string`"))
    #expect(propertyDescription(named: "note", in: replace)?.contains("exact case-insensitive title across notes and archive") == true)
    #expect(propertyDescription(named: "kind", in: replace)?.contains("`frontmatter` replaces or removes only the Bear YAML frontmatter block") == true)
    #expect(propertyDescription(named: "kind", in: replace)?.contains("Do not use `body` to add or edit frontmatter") == true)
    ...
    #expect(try #require(getNotes.description).contains("returns that frontmatter separately from the editable `content` as `frontmatter.content`"))
