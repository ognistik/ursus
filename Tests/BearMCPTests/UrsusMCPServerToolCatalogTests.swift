...
func toolCatalogInjectsCurrentSessionDefaultsIntoOverrideableFields() throws {
    ...
    #expect(createDescription.contains("creation receipt may also include the note's current Bear `version`, whether the note was opened, and whether it opened in the main Bear window or a new window"))
    ...
    #expect(try #require(replace.description).contains("server-issued `conflict_token`"))
    #expect(try #require(replace.description).contains("`kind: front_matter`"))
    #expect(try #require(replace.description).contains("use `kind: front_matter`, not `kind: body`"))
    #expect(try #require(replace.description).contains("ignored for `kind: title`, `kind: front_matter`, and `kind: string`"))
    #expect(propertyDescription(named: "note", in: replace)?.contains("exact case-insensitive title across notes and archive") == true)
    #expect(propertyDescription(named: "kind", in: replace)?.contains("`front_matter` replaces or removes only the Bear YAML front matter block") == true)
    #expect(propertyDescription(named: "kind", in: replace)?.contains("Do not use `body` to add or edit front matter") == true)
    #expect(propertyDescription(named: "new_string", in: replace)?.contains("omit the surrounding `---` fences") == true)
    #expect(propertyDescription(named: "expected_version", in: replace)?.contains("Required only for `kind: body`") == true)
    #expect(propertyDescription(named: "expected_version", in: replace)?.contains("ignored") == true)
    #expect(propertyDescription(named: "conflict_token", in: replace)?.contains("short-lived, single-use") == true)
    ...
    #expect(try #require(getNotes.description).contains("returns that front matter separately from the editable `content` as `frontMatter.content`"))
