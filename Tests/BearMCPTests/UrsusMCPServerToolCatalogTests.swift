...
func toolCatalogInjectsCurrentSessionDefaultsIntoOverrideableFields() throws {
    ...
    #expect(createDescription.contains("omitted `open_note` uses `false`"))
    #expect(createDescription.contains("omitted `new_window` uses `false`"))
    #expect(createDescription.contains("configured inbox tags are `0-inbox`, `client work`"))
    #expect(createDescription.contains("tag merging on omission currently uses request tags when any are supplied"))
    ...
    #expect(insertDescription.contains("`position` uses `top`"))
    #expect(insertDescription.contains("`new_window` uses `false` when the note is opened"))
    ...
    #expect(addFilesDescription.contains("`position` uses `top`"))
    ...
    #expect(try #require(openNotes.description).contains("Current omission default: `new_window` uses `false`"))
